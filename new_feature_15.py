...
def main():
	...
	new_columns_name = ['e_p2_m_t', 'e_i_t', 'p_avr_10min', 'air_density_20', 'p1_a_10med',
       ...
       'e_p2_ng_t', 'g_s_avr_10max', 'wind_speed_2', 'e_tmp_10med']
	#先把相应的列挑出来，然后再删除某些行
...
def cal_statist(train_data):
	...
	max_10_over_array = np.transpose(np.reshape(temp_array,(20,393886)))
	i = 10
	j='max'
	...
	min_10_over_array = np.transpose(np.reshape(temp_array,(20,393886)))
	i = 10
	j='min'
	...
	std_10_over_array = np.transpose(np.reshape(temp_array,(20,393886)))
	i = 10
	j='std'
	...
	med_10_over_array = np.transpose(np.reshape(temp_array,(20,393886)))
	i = 10
	j='med'
	...
	avr_10_over_array = np.transpose(np.reshape(temp_array,(20,393886)))
	i = 10
	j='avr'
